    ...
    .map((p) =>
      render(CARD, {
        this: {
          ...
          shotHtml: p.shot
            ? `<img class="shot" src="/assets/shots/${p.shot}" alt="${esc(p.name)} screenshot" loading="lazy" width="1440" height="900">`
            : '',
          linksHtml: p.links.length
            ? `<p class="links">${p.links.map((l) => `<a href="${esc(l.url)}">${esc(l.label)}</a>`).join(' · ')}</p>`
